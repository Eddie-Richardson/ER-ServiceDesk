# ER-ServiceDesk/app/crud/note_template.py
"""
...
class NoteTemplateCRUD:
    """Direct database access for NoteTemplate records."""

    def get(self, db: Session, id: int) -> NoteTemplate | None:
        return db.query(NoteTemplate).filter(NoteTemplate.id == id).first()

    def get_multi(self, db: Session, skip: int = 0, limit: int = 100):
        return db.query(NoteTemplate).offset(skip).limit(limit).all()

    def create(self, db: Session, obj_in: NoteTemplateCreate) -> NoteTemplate:
        obj = NoteTemplate(**obj_in.model_dump())
        db.add(obj)
        db.commit()
        db.refresh(obj)
        return obj

    def update(self, db: Session, db_obj: NoteTemplate, obj_in: NoteTemplateUpdate) -> NoteTemplate:
    ...
    def delete(self, db: Session, id: int) -> None:
        obj = db.query(NoteTemplate).filter(NoteTemplate.id == id).first()
        if obj:
            db.delete(obj)
            db.commit()

crud_note_template = NoteTemplateCRUD()
