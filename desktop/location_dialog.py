# ER-ServiceDesk/desktop/location_dialog.py

"""
Dialog for creating a new location or editing an existing one.
"""
...
class LocationDialog(AppDialog):
    """
    Modal dialog for creating or editing a location.

    Pass `location=None` to create a new one, or an existing location
    dict to edit it. On a successful save, the dialog closes itself
    and the saved record is available via `self.saved_location`.
    """

    def __init__(self, location: dict | None = None, parent=None):
        super().__init__(parent)
        self.location = location
        self.saved_location: dict | None = None
        ...
        restore_geometry(self, "LocationDialog")

        self._build_ui()
        if location:
            self._prefill_from_location(location)

    def closeEvent(self, event):
        save_geometry(self, "LocationDialog")
        super().closeEvent(event)

    def _build_ui(self):
        """Builds the Name, Description, and Show in Ticket Location Picker fields."""
        ...
        self.name_input.setPlaceholderText("Name, e.g. 'Bench 1' (required)")
        ...
        self.show_in_ticket_picker_checkbox = QCheckBox("Show in ticket location picker")
        self.show_in_ticket_picker_checkbox.setChecked(True)
        ...
        outer_layout.addWidget(self.show_in_ticket_picker_checkbox)
    ...
    def _prefill_from_location(self, location: dict):
        self.name_input.setText(location.get("name", ""))
        self.description_input.setPlainText(location.get("description") or "")
        self.show_in_ticket_picker_checkbox.setChecked(location.get("show_in_ticket_picker", True))
    ...
    def _attempt_save(self):
        ...
        payload = {
            ...
            "show_in_ticket_picker": self.show_in_ticket_picker_checkbox.isChecked(),
        }
        ...
        self._worker = LookupSaveWorker(ENDPOINT, payload, location_id)
